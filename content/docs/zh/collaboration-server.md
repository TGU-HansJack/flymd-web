...
本文档介绍如何搭建 flyMD 协同服务器，实现多人实时协作编辑。
...
```json
// 加入房间（首次连接时发送）
{"type":"join","content":"当前全文内容"}

// 文本更新
{"type":"update","content":"当前全文内容"}

// 请求锁定某一行/段
{"type":"lock","blockId":"b_xxx","label":"标题","color":"#rrggbb"}
...
```json
// 加入房间后的初始快照
{"type":"snapshot","content":"全文内容"}

// 其他客户端更新后的最新内容
{"type":"update","content":"全文内容"}
...
填写后点击"连接"，插件会与服务器建立 WebSocket 连接并开始同步当前文档内容。

## 注意事项

- 所有房间信息与内容仅保存在内存中，**重启进程后会全部丢失**
- 未做账号体系，仅按 `room + password` 区分空间
- 建议在生产环境中使用 HTTPS/WSS 并配置适当的访问控制

## 获取服务器代码

服务器代码位于 flyMD 仓库的 `OSserver` 目录中：
